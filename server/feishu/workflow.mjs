...
export function createFeishuWorkflow({ store, ocr, extractor, writeback }) {
  ...
  async function run(taskId) {
    ...
    try {
      await store.update(taskId, {
        ...
        progress: { current: 1, total: 4, label: '飞书任务已触发 PaddleOCR / PP-Structure' }, error: null,
      }, 'ocr_started')
  ...
  async function createTask(input) {
    ...
    if (!created.reused) queueMicrotask(() => { void run(created.task.taskId) })
    return created
  }
  ...
  async function confirmAndWrite(taskId, inputLocations) {
    ...
    try {
      const outputs = await writeback.write(store.getInternal(taskId), locations)
